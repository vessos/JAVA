...
// private HashMap<String, HashMap<String, ArrayList<Integer>>> studentsByCourse;
            ...
            if (!line.isEmpty() && matcher.find()) {
                ...
                try {
                    ...
                    if(Arrays.stream(scores).anyMatch(score->score>100||score<0)){
                        OutputWriter.displayException("The number for the score you've entered is not in the range of 0 - 100");
                        continue;
                    }
                    ...
                    course.enrollStrudent(student);

                }catch(NumberFormatException nfe){
                    OutputWriter.displayException(nfe.getMessage()+" at time: "+ line);
                }
    ...
    public void getStudentMarksInCourse(String courseName, String studentName) {
        ...
        double mark = this.courses.get(courseName).studentByName
                .get(studentName).marksByCourseName
                .get(courseName);
    ...
    public void getStudentsByCourse(String courseName) {
        ...
        for (Map.Entry<String, Student> student : this.courses.get(courseName).studentByName.entrySet()) {
    ...
    private boolean isQueryForStudentPossible(String courseName, String studentName) {
        ...
        if (!this.courses.get(courseName).studentByName.containsKey(studentName)) {
    ...
    public void filterAndTake(String courseName, String filter) {
        int studentsToTake = this.courses.get(courseName).studentByName.size();
    ...
    public void filterAndTake(
        ...
        for (Map.Entry<String,Student>entry : this.courses.get(courseName).studentByName.entrySet()) {
            marks.put(entry.getKey(),entry.getValue().marksByCourseName.get(courseName));
        }
        this.sorter.printSortedStudents(marks,filter,studentsToTake);
    }
